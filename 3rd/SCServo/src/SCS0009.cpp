...
{
	if (ID == (u8) -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SCS0009_PRESENT_SPEED_L - SCS0009_PRESENT_POSITION_L,
...
{
	if (ID == (u8) -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SCS0009_PRESENT_LOAD_L - SCS0009_PRESENT_POSITION_L,
		                                            SCS0009_PRESENT_LOAD_H - SCS0009_PRESENT_POSITION_L,
		                                            SCS0009_DIRECTION_BIT_POS);
	}
	this->Err = 0;
	int load = this->readSignedWord(ID, SCS0009_PRESENT_LOAD_L, SCS0009_DIRECTION_BIT_POS);
	if (load == -1)
		this->Err = 1;
	return load;
}
...
{
	if (ID == (u8) -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SCS0009_PRESENT_CURRENT_L - SCS0009_PRESENT_POSITION_L,
		                                            SCS0009_PRESENT_CURRENT_H - SCS0009_PRESENT_POSITION_L,
		                                            SCS0009_DIRECTION_BIT_POS);
	}
	this->Err = 0;
	int current = this->readSignedWord(ID, SCS0009_PRESENT_CURRENT_L, SCS0009_DIRECTION_BIT_POS);
	if (current == -1)
		this->Err = 1;
	return current;
}
