...
Si12T::Si12T(const char* device, uint8_t i2c_addr)
    : device_(device), addr_(i2c_addr) {}
...
bool Si12T::writeReg(uint8_t reg, uint8_t value) {
    ...
    return fd_ >= 0 && write(fd_, data, sizeof(data)) == sizeof(data);
}

bool Si12T::readReg(uint8_t reg, uint8_t* out) {
    if (out == nullptr || fd_ < 0) return false;
    return write(fd_, &reg, sizeof(reg)) == sizeof(reg) &&
           read(fd_, out, sizeof(*out)) == sizeof(*out);
}
...
bool Si12T::begin(SensitivityType sens_type, SensitivityLevel sens_level) {
    ...
    if (fd_ < 0) fd_ = open(device_, O_RDWR);
    if (fd_ < 0 || ioctl(fd_, I2C_SLAVE, addr_) < 0) {
        if (fd_ >= 0) {
            close(fd_);
            fd_ = -1;
        }
